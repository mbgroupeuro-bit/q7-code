...
interface Props {
  ...
  zielId: string;
  titel: string;
  vorschlagText?: string;
}

const POLLING_INTERVALL_MS = 4000;

export default function ChatFenster({ typ, zielId, titel, vorschlagText }: Props) {
  const [nachrichten, setNachrichten] = useState<ChatNachrichtDto[]>([]);
  const [text, setText] = useState(vorschlagText ?? "");
